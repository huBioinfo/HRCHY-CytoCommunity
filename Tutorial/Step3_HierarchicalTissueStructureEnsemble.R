...
CoarseMatrix <- matrix(data = NA, nrow=tcn1_number)
...
for (i in 1:tcn2_number){
  finalmatrix[finalmatrix == i] = tcn1_number+i
}
for (i in (tcn2_number+1):tcn1_number){
  finalmatrix[finalmatrix == i] = trans[i,1]
}
for (i in 1:tcn2_number){
  finalmatrix[finalmatrix == tcn1_number+i] = trans[i,1]
}
